...
module.exports = (client, app) => {
    ...
    async function connectDB() {
        try {
            ...
            console.log("📂 [SociaBuzz] Database Connected!");
        } catch (e) {
    ...
    app.post('/webhook', async (req, res) => {
        // STRATEGI: Langsung kirim 200 OK biar SociaBuzz nggak nungguin proses bot yang berat
        res.status(200).send('OK');

        // Proses sisanya dijalanin secara asinkron di background
        (async () => {
            try {
                const data = req.body;
                        ...
                        components: [
                            {
                                ...
                                components: [
                                    ...
                                    { type: 14 },
                                    {
                                        ...
                                        accessory: { type: 2, style: 5, label: "Donasi", url: "https://sociabuzz.com/bananaskiee/donasi }
                                    },
                                    { type: 14 },
                                    {
                                        type: 1,
                ...
                console.error("🚨 [SociaBuzz] Background Process Error:", err);
            }
        })();
    });
    ...
    client.on('interactionCreate', async (interaction) => {
        ...
        try {
            ...
            if (existingLike) return interaction.reply({ content: "Kamu sudah memberikan dukungan ❤️!", ephemeral: true });
            ...
            // Indeks tombol Like: Section -> Row -> Button ke-0
            oldComponents[0].components[4].components[0].label = `${likeCount}`;

            await interaction.update({ components: oldComponents });
        } catch (err) {
            console.error("❌ Like Interaction Error:", err);
        }
    });

    console.log("✅ SociaBuzz Module (Fast Response Mode) Loaded");
};
