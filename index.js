...
// ========== SOCIABUZZ WEBHOOK (harus sebelum listen) ==========
const sociabuzz = require("./modules/sociabuzz");
try {
  sociabuzz(client, app);
  console.log("✅ Sociabuzz webhook route registered");
} catch (err) {
  console.error("❌ Gagal register sociabuzz:", err);
}
...
client.on("messageCreate", guard("messageCreate", async (message) => {
  ...
  try { await handleSuggestionMessage(message); } catch (err) { console.error("Suggestion error:", err); }

  if (message.content.toLowerCase() === "bs!verify invite") {
    try {
      if (client.verifyInviteSystem) await client.verifyInviteSystem.handleVerifyCommand(message);
      else message.reply("❌ Sistem verify belum siap!");
    } catch (err) { message.reply("❌ Error saat verify!"); }
    return;
  }
...
client.on("ready", async () => {
  ...
  if (targetGuild) {
    try {
      ...
      console.log(`✅ Invite cache ready for ${targetGuild.name}`);
    } catch (error) {
      console.error("❌ Gagal fetch invites:", error.message);
    }
  }
});
...
client.on("guildMemberAdd", guard("guildMemberAdd", async (member) => {
  ...
  } catch (error) { console.error("INVITE TRACKER error:", error.message); }
}));

client.on("guildMemberRemove", guard("guildMemberRemove", async (member) => {
  firstMessageCache.delete(member.id);
  if (client.verifyInviteSystem) await client.verifyInviteSystem.trackLeave(member);
}));
...
client.on("webhookUpdate", guard("webhookUpdate", async () => {}));

client.login(config.token);

process.on("SIGTERM", async () => {
  console.log("🛑 SIGTERM received");
  if (client.verifyInviteSystem) await client.verifyInviteSystem.close();
  try {
    const logChannel = await client.channels.fetch("1352800131933802547");
    if (logChannel) {
      await logChannel.send({ embeds: [{ color: 0xe74c3c, description: "🛑 **Status:** Offline / Sedang Restart.", timestamp: new Date() }] });
    }
  } catch (err) {}
  client.destroy();
